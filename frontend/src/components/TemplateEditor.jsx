...
export default function TemplateEditor({ accent = ACCENT }) {
  ...
  useEffect(() => {
    if (notFound) navigate('/settings/autopost', { replace: true })
  }, [notFound, navigate])
  ...
  function handleSave() {
    ...
    navigate('/settings/autopost')
  }

  return (
    <div className="tpl-page" style={{ '--ing-accent': accent }}>
      <div className="tpl-header" ref={headerRef}>
        <button type="button" className="tpl-back-btn" onClick={() => navigate('/settings/autopost')}>
          <BackIcon />
          Back
        </button>
